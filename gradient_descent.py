...
for epoch in np.arange(0, args["epochs"]):
	...
	# 'error', which is the difference between our bedictions and 
	# the true values
	error = preds - trainY
	loss = np.sum(error ** 2)
	losses.append(loss)
...
# plt.scatter(testX[:, 0], testX[:, 1], c=testY, marker="o", s=30) # <- !!!this wouldn't work, cause testY has to been reshaped from 2d array to 1d array
plt.scatter(testX[:, 0], testX[:, 1], c=testY.reshape(testY.shape[0]), marker="o", s=30)
