...
ap.add_argument("-a", "--annot", required=True,
	help="path to ouput directory of annotations")
args = vars(ap.parse_args())

# grab the image paths then initialize the dictionary of character
# counts
imagePaths = list(paths.list_image(args["input"]))
counts = {}
...
for (i, imagePath) in enumerate(imagePaths):
	...
	try:
		...
		for c in cnts:
			...
			# if the ''' is pressed, then ignore the charactor
			if key = ord("'"):
			...
			# write the labeled charactor to file
	...
	# an unknown error has occoured for this particular image
	except:
