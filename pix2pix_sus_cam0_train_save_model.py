...
datasetpath = '.\\SUS_Bend\\output\\datasetimages.hdf5'
patch_size = 64	#32
batch_size = 12
epoch = 300
...
ap.add_argument("-m", "--model", required=True,
	help="path to save generator model")
...
def DCGAN_discriminator(img_shape, disc_img_shape, patch_num, model_name='DCGAN_discriminator'):
	# discriminator raw image shape use the W, H of discriminator image, and the Channel depth of raw image
	disc_raw_img_shape = (disc_img_shape[0], disc_img_shape[1], img_shape[-1])
	# input from generator and patched
	list_input = [Input(shape=disc_img_shape, name='disc_input_'+str(i)) for i in range(patch_num)]
	# input from raw image and patched
...
def load_DCGAN_discriminator(img_shape, disc_img_shape, pathc_num):
	model = DCGAN_discriminator(img_shape, disc_img_shape, pathc_num)
	return model
...
def get_disc_batch(procImage, rawImage, generator_model, batch_counter, patch_size):
	...
	if batch_counter % 2 == 0:
		# produce an output\\datasetimages
...
def train():
	...
	# patch_num = patch num in w * patch num in h
	patch_num = (img_shape[0] // patch_size) * (img_shape[1] // patch_size)
	# discriminator image shape, w,h=patch_size, color channel = procImage channel
	...
	generator_model.save(args["model"])
